...
def send_otp():
    global token
    token = generate_token_OTP(OTP_MOBILE_NUMBER,base_request_header)

if __name__ == '__main__':
    sched = BackgroundScheduler(daemon=True)
    sched.add_job(send_otp, 'interval', minutes=3)
    sched.start()
    main()
